  ...
  for (const c of body.children ?? []) {
    stmts.push(env.DB.prepare(
      `INSERT INTO children (id, family_id, name, birth_date, risk_tier, jurisdiction,
         readiness_confirmed_on, clinician_cleared, excluded, scheduled,
          exposure_counts, settings, updated_at)
       ...
       ON CONFLICT(id) DO UPDATE SET
         ...
         scheduled=excluded.scheduled, exposure_counts=excluded.exposure_counts,
       ...
       WHERE excluded.updated_at > children.updated_at`
           ...
           c.excluded ?? "[]", c.scheduled ?? "[]", c.exposure_counts ?? "{}",
           c.settings ?? "{}", Number(c.updated_at ?? now)));
  }
